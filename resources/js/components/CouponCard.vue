      ...
      <div class="modal-content">
              ...
              >
                ...
                >
                  ...
                  <label
                    ...
                    :for="coupon.code + coupon.id"
                  >
                    <span class="h6 d-block mb-2">{{ coupon.code }}</span>
                    <span class="d-block mb-1">
                      <span
                        v-if="coupon.discount_type == 'fixed'"
                        class="d-block small"
                      >
                        ${{ coupon.discount }} {{ $t('landingpage.off') }}
                      </span>
                      <span v-else class="d-block small">
                        {{ coupon.discount }}% {{ $t('landingpage.off') }}
                      </span>
                    </span>
                    <span
                    ...
                    >
                      {{ $t('landingpage.exp') }}:
                      {{ moment(coupon.expire_date).format('D MMM, YYYY') }}
                    </span>
                  </label>
                </div>
              </div>
        ...
        <div class="modal-footer">
          <button
            ...
            v-if="is_applied == 1"
            @click="getSelectedCoupon()"
          >
            {{ $t('landingpage.applied') }}
          </button>
          <button
            class="btn btn-primary"
            v-if="is_applied != 1"
            :disabled="!(is_applied === 0 && !!couponId)"
            @click="getSelectedCoupon()"
          >
            {{ $t('messages.apply') }}
          </button>
...
const selectCoupon = (selected_id) => {
  couponId.value = selected_id
  is_applied.value = (couponId.value === props.SeletedCouponId) ? 1 : 0
}
...
const filteredCoupons = computed(() => {
  return props.coupons.filter(coupon => {
    ...
    if (coupon.discount_type === 'percent') {
      discountAmount = (coupon.discount / 100) * props.service_price
    }
    return discountAmount <= props.service_price
  })
})
</script>
