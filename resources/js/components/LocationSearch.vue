                                    ...
                                    <a :href="`${baseUrl}/service-detail/${service.id}`">
                                       <div class="post-img d-flex align-items-center">
                                          <div class="post-img-holder">
                                             <a :href="`${baseUrl}/service-detail/${service.id}`">
                                                <img
                                                   :src="service.attchments[0] ? service.attchments[0] : baseUrl + '/images/default.png'"
                                                   alt="">
                                             </a>
                                          </div>
               ...
               <div class="modal-body">
                  <form>
                     ...
                     <div class="row row-cols-2 pt-4">
                        ...
                        <div v-if="user_lat != null && user_lng != null" class="col">
                           <button type="button" class="btn btn-primary w-100" @click="removeCurrentLocation">{{
...
import { CATEGORY_API, SERVICE_API, POST_SERVICE_API, REMOVE_SESSION,SET_SESSION } from '../data/api';
import { useSection } from '../store/index'
//import {useObserveSection} from '../hooks/Observer'
const props = defineProps(['postjobservice', 'user_id', 'user_lat', 'user_lng']);
...
const Location_name = ref(localStorage.getItem('loction_name') || null);
         ...
         async (position) => {
            ...
            localStorage.setItem('is_location_on', 'on');
              ...
              if (location) {
               ...
               const response = await fetch(`${SET_SESSION}?lat=${currentLatitude}&lng=${currentLongitude}`);
                 const data = await response.json();
...
const getLocationName = async (latitude, longitude) => {
   ...
   try {
      ...
      if (data && data.display_name) {
              localStorage.setItem('loction_name', data.display_name);
