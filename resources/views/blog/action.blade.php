...
?>
{{ html()->form('DELETE', route('blog.destroy', $blog->id))->attribute('data--submit', 'blog'.$blog->id)->open()}}
<div class="d-flex justify-content-end align-items-center">
    @if(!$blog->trashed())
        <!-- @if($auth_user->can('blog edit'))
        <a class="me-2" href="{{ route('blog.create',['id' => $blog->id]) }}" title="{{ __('messages.update_form_title',['form' => __('messages.blog') ]) }}"><i class="fas fa-pen text-primary"></i></a>
        @endif -->

        @if($auth_user->can('blog delete'))
        <a class="me-3" href="{{ route('blog.destroy', $blog->id) }}" data--submit="blog{{$blog->id}}" 
