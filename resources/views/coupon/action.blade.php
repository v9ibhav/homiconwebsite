...
?>
{{ html()->form('DELETE', route('coupon.destroy', $coupon->id))->attribute('data--submit', 'coupon'.$coupon->id)->open()}}
<div class="d-flex justify-content-end align-items-center">
    @if(!$coupon->trashed())
        <!-- @if($auth_user->can('coupon edit'))
        <a href="{{ route('coupon.create',['id' => $coupon->id]) }}" title="{{ __('messages.update_form_title',['form' => __('messages.coupon') ]) }}"><i class="fas fa-pen text-primary me-2"></i></a>
        @endif -->
        @if($auth_user->can('coupon delete'))
        <a class=" me-3" href="{{ route('coupon.destroy', $coupon->id) }}" data--submit="coupon{{$coupon->id}}" 
