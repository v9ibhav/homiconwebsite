<x-master-layout>
    <div class="container-fluid">
        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body">
                    {{ html()->form('POST', route('about-us-save'))->attribute('data-toggle', 'validator')->open() }}
                    ...
                    @foreach($language_array as $language)
                        <div id="form-language-{{ $language['id'] }}" class="language-form" style="display: {{ $language['id'] == app()->getLocale() ? 'block' : 'none' }};">
                            <div class="row">
                                @foreach(['value' => __('messages.about_us')] as $field => $label)
                                    <div class="form-group col-md-{{ $field === 'value' ? '12' : '12' }}">
                                        ...
                                        @if($field === 'value')
                                            {{ html()->textarea($name, $value)
                                                ->class('form-control tinymce-about_us')
                                                ->rows(3)
                                                ->placeholder($label) }}
                                        @endif
        ...
        <script>
            (function($) {
                $(document).ready(function() {
                    tinymceEditor('.tinymce-about_us', ' ', function(ed) {}, 450);
                });
            })(jQuery);
        </script>
    @endsection
