        ...
        <div class="card card-block card-stretch">
          <div class="card-body p-0">
            <div class="d-flex justify-content-between align-items-center p-3 flex-wrap gap-3">
              ...
              <a href="{{ route('provideraddress.create',['provideraddress' => $providerdata->id]) }}" class=" float-end me-1 btn btn-sm btn-primary"><i class="fa fa-plus-circle"></i> {{ trans('messages.add_form_title',['form' => trans('messages.provider_address')  ]) }}</a>
              @endif
            </div>
      ...
      <div class="card-body">
        <div class="row justify-content-between gy-3">
          <div class="col-md-6 col-lg-4 col-xl-3">
            <div class="col-md-12">
              <form action="{{ route('provideraddress.bulk-action') }}" id="quick-action-form" class="form-disabled d-flex gap-3 align-items-center">
                @csrf
                ...
                <button id="quick-action-apply" class="btn btn-primary" data-ajax="true"
                  data--submit="{{ route('provideraddress.bulk-action') }}"
                  data-datatable="reload" data-confirmation='true'
                  data-title="{{ __('provideraddress',['form'=>  __('provideraddress') ]) }}"
                  title="{{ __('provideraddress',['form'=>  __('provideraddress') ]) }}"
  ...
  <script>
    document.addEventListener('DOMContentLoaded', (event) => {
      ...
      window.renderedDataTable = $('#datatable').DataTable({
        ...
        ajax: {
          "type": "GET",
          "url": '{{ route("provideraddress.index_data", ["provideraddress" => $providerdata->id]) }}',
          "data": function(d) {
            d.search = {
              value: $('.dt-search').val()
            };
